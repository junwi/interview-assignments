...
describe('Integration test', () => {
    function memcachedFlush(memcached): Promise<void> {
    ...
    describe('post /shorten', () => {
        test('when input is empty.', async () => {
            const res = await request(server).post('/shorten').send({ 'url': '' });
        ...
        test('when input is too long.', async () => {
            ...
            const res = await request(server).post('/shorten').send({ 'url': input });
            expect(res.statusCode).toBe(400);
        ...
        test('when input is a valid url.', async () => {
            const res = await request(server).post('/shorten').send({ 'url': 'https://baidu.com' });
        ...
        test('when first code is coccupied, pick first unused.', async () => {
            const res = await request(server).post('/shorten').send({ 'url': 'https://hello.com' });
            expect(res.statusCode).toBe(200);
            expect(res.body).toHaveProperty('code', 'oZpUFIKZ');
        });
        test('when first code is coccupied, another code matched.', async () => {
            const res = await request(server).post('/shorten').send({ 'url': 'https://google.com' });
            expect(res.statusCode).toBe(200);
    ...
    describe('test /:code', () => {
        test('when input is not a valid short url.', async () => {
            const res = await request(server).get('/somekey');
            expect(res.statusCode).toBe(400);
        ...
        test('when code is correct.', async () => {
            const res = await request(server).get('/u24ILVw2');
            expect(res.statusCode).toBe(200);
